  ...
  const rows = useMemo(() => {
    ...
    data.forEach(c => {
      ...
      if (!map[key]) map[key] = {
        ...
        monto: 0, repDias: [], verde: 0, amarillo: 0, rojo: 0
      }
      map[key].total++
      const cd = Number(c.cd_estatus)
      if (CLOSED_STATUSES.includes(cd)) map[key].cerrados++
    ...
    return Object.values(map)
      .map(r => ({
        ...r,
        promRep: r.repDias.length
          ? Math.round(r.repDias.reduce((a, b) => a + b, 0) / r.repDias.length)
          : null,
      }))
      .sort((a, b) => {
  ...
  const chartData = useMemo(() =>
    qualified
      ...
      .map(r => ({
        ...
        Casos: r.total,
        'Prom. Reparación (días)': r.promRep ?? 0,
      })), [qualified])

  function handleRowClick(taller) {
    navigate(`/detalle?taller=${encodeURIComponent(taller)}`)
  }
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead style={{ background: '#F8FAFC' }}>
              <tr>
                {['Taller', 'Total', 'Activos', 'Cerrados', 'Prom. Reparación', 'Monto Est.', '🟢', '🟡', '🔴'].map(h => (
            ...
            <tbody>
              {qualified.map(r => (
                <tr
                  key={r.taller}
                  onClick={() => handleRowClick(r.taller)}
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span className="text-xs font-medium" style={{ color: '#B91C1C' }}>{r.rojo}</span>
                  </td>
                </tr>
              ))}
            </tbody>
